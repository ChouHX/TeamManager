    ...
    try:
        ...
        for team in teams_result.get("teams", []):
            if team.get("expires_at"):
                try:
                    team["expires_at"] = datetime.fromisoformat(team["expires_at"]).strftime("%Y-%m-%d %H:%M")
                except Exception:
                    pass
...
async def team_members_list(
    ...
    try:
        ...
        if result.get("success"):
            ...
            team = query_result.scalar_one_or_none()
            if team:
                team.current_members = result.get("total", 0)
...
async def export_codes(
    ...
    try:
        ...
        # 获取所有兑换码 (导出不分页，传入大数量)
        codes_result = await redemption_service.get_all_codes_for_export(db)
        if not codes_result.get("success"):
            raise RuntimeError(codes_result.get("error") or "查询兑换码失败")
...
async def records_page(
    ...
    try:
        ...
        if not records_result["success"]:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=records_result["error"]
            )

        stats = await redemption_service.get_record_stats(
            db,
            email=email,
            code=code,
            team_id=actual_team_id,
        ...
        return templates.TemplateResponse(
            "admin/index.html",
            {
                ...
                "pagination": {
                    "current_page": records_result["current_page"],
    ...
    try:
        ...
        return templates.TemplateResponse(
            "admin/index.html",
            {
                ...
                "title": "系统设置",
                "message": "管理代理、日志级别、Webhook 与 API Key。",
                "settings": {
                    ...
                    "linuxdo_credit_enabled": await settings_service.get_setting(db, "linuxdo_credit_enabled", "false"),
                    "linuxdo_credit_pid": await settings_service.get_setting(db, "linuxdo_credit_pid", ""),
                    "linuxdo_credit_key": await settings_service.get_setting(db, "linuxdo_credit_key", ""),
                    "linuxdo_credit_price": await settings_service.get_setting(db, "linuxdo_credit_price", ""),
                    "team_ban_poll_interval_minutes": await settings_service.get_team_ban_poll_interval_minutes(db),
                    "linuxdo_credit_notify_url_display": f"{str(request.base_url).rstrip('/')}/api/credit/notify",
                    "linuxdo_credit_callback_uri_display": f"{str(request.base_url).rstrip('/')}/credit/callback",
                    "linuxdo_credit_default_name": await settings_service.get_setting(db, "linuxdo_credit_default_name", "Linux.do 席位购买")
                }
            }
        )
...
class WebhookSettingsRequest(BaseModel):
    ...
    linuxdo_credit_enabled: bool = Field(False, description="是否启用 Linux.do 积分购席位")
    linuxdo_credit_pid: str = Field("", description="Linux.do Credit Client ID")
    linuxdo_credit_key: str = Field("", description="Linux.do Credit Client Secret")
    linuxdo_credit_price: str = Field("", description="Linux.do Credit 固定价格")
    linuxdo_credit_default_name: str = Field("Linux.do 席位购买", description="Linux.do 席位标题")


class TeamBanPollingSettingsRequest(BaseModel):
    """Team 封禁巡检设置请求"""
    interval_minutes: int = Field(30, description="轮询间隔(分钟)")
...
@router.post("/settings/team-ban-polling")
async def update_team_ban_polling_settings(
    polling_data: TeamBanPollingSettingsRequest,
    db: AsyncSession = Depends(get_db),
    current_user: dict = Depends(require_admin)
):
    """更新 Team 封禁巡检轮询设置"""
    try:
        from app.services.settings import (
            settings_service,
            MIN_TEAM_BAN_POLL_INTERVAL_MINUTES,
            MAX_TEAM_BAN_POLL_INTERVAL_MINUTES,
        )

        interval_minutes = int(polling_data.interval_minutes)
        logger.info(f"管理员更新 Team 封禁巡检轮询间隔: {interval_minutes} 分钟")

        success = await settings_service.update_team_ban_poll_interval_minutes(db, interval_minutes)

        if success:
            return JSONResponse(content={"success": True, "message": "轮询设置已保存"})

        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={
                "success": False,
                "error": f"轮询间隔必须在 {MIN_TEAM_BAN_POLL_INTERVAL_MINUTES}-{MAX_TEAM_BAN_POLL_INTERVAL_MINUTES} 分钟之间"
            }
        )

    except Exception as e:
        logger.error(f"更新 Team 封禁巡检轮询设置失败: {e}")
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"success": False, "error": f"更新失败: {str(e)}"}
        )
...
async def update_webhook_settings(
    ...
    try:
        ...
        settings = {
            ...
            "linuxdo_credit_enabled": "true" if webhook_data.linuxdo_credit_enabled else "false",
            "linuxdo_credit_pid": webhook_data.linuxdo_credit_pid.strip(),
            "linuxdo_credit_key": webhook_data.linuxdo_credit_key.strip(),
            "linuxdo_credit_price": webhook_data.linuxdo_credit_price.strip(),
            "linuxdo_credit_default_name": webhook_data.linuxdo_credit_default_name.strip() or "Linux.do 席位购买"
        }
