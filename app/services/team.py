...
class TeamService:
    ...
    async def _sync_team_member_count(self, team: Team, member_count: int, db_session: AsyncSession) -> None:
        """根据最新成员和邀请数量回写 Team 统计。"""
        team.current_members = member_count
        if team.status in ["active", "full", "expired", "error"]:
    ...
    async def sync_team_info(
        ...
        try:
            ...
            # 成员和邀请列表互不依赖，并发拉取可以减少同步总耗时
            members_result, invites_result = await asyncio.gather(
    ...
    async def probe_team_ban_status(
        self,
        team: Team,
        db_session: AsyncSession
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not access_token:
                ...
                return {
                    ...
                    "email": team.email,
                    "status": team.status,
                    "error": "Token 宸茶繃鏈熶笖鏃犳硶鍒锋柊"
                }

            token_email = self.jwt_parser.extract_email(access_token)
            if token_email and team.email and token_email.lower() != team.email.lower():
                logger.error(
                    "Team %s ban probe token email mismatch, expected=%s actual=%s",
                    team.id,
                    team.email,
                    token_email,
                )
                ...
                return {
                    ...
                    "email": team.email,
                    "status": team.status,
                    "error": f"鍒锋柊鍑虹殑璐﹀彿韬唤 ({token_email}) 涓庡師璐﹀彿 ({team.email}) 涓嶇"
                }
    ...
    async def poll_team_ban_statuses(
        self,
        db_session: AsyncSession
    ) -> Dict[str, Any]:
        """轮询全部未封禁 Team 的封禁状态。"""
        try:
            result = await db_session.execute(
                select(Team).where(Team.status != "banned").order_by(Team.id.asc())
            )
            teams = result.scalars().all()
            ...
            for team in teams:
                ...
                probe_result = await self.probe_team_ban_status(team, db_session)
                checked_count += 1

                if team.status == "banned" and previous_status != "banned":
                    banned_count += 1

                if not probe_result["success"] and team.status != "banned":
                    failed_count += 1
            ...
            logger.info(
                "Team ban polling finished: total=%s checked=%s banned=%s failed=%s",
                len(teams),
                checked_count,
                banned_count,
                failed_count,
            ...
            logger.error(f"Team ban polling failed: {e}")
            return {
                "success": False,
                "total": 0,
                "checked_count": 0,
