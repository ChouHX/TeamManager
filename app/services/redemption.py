...
class RedemptionService:
    ...
    def _build_record_filters(
        ...
        if end_date:
            ...
            filters.append(RedemptionRecord.redeemed_at < end_dt)
    ...
    async def get_all_codes_for_export(
        self,
        db_session: AsyncSession
    ) -> Dict[str, Any]:
        """获取全部兑换码, 用于导出, 不受当前筛选条件影响。"""
        try:
            stmt = select(RedemptionCode).order_by(RedemptionCode.created_at.desc())
            result = await db_session.execute(stmt)
            codes = result.scalars().all()
            ...
            logger.error(f"导出兑换码查询失败: {e}")
            return {
                "success": False,
                "codes": [],
                "total": 0,
                "error": f"导出兑换码查询失败: {str(e)}"
            }
    ...
    async def get_records_page(
        ...
        """分页获取兑换记录，并在 SQL 层完成过滤。"""
        try:
            filters = self._build_record_filters(email, code, team_id, start_date, end_date)

            count_stmt = select(func.count()).select_from(RedemptionRecord)
            if filters:
                count_stmt = count_stmt.where(and_(*filters))
            count_result = await db_session.execute(count_stmt)
            total = count_result.scalar() or 0
            ...
            stmt = select(RedemptionRecord, Team.team_name).outerjoin(
                Team, RedemptionRecord.team_id == Team.id
            )
            if filters:
                stmt = stmt.where(and_(*filters))
            stmt = stmt.order_by(RedemptionRecord.redeemed_at.desc()).limit(per_page).offset((page - 1) * per_page)
    ...
    async def get_record_stats(
        ...
        """获取兑换记录统计，避免先全量加载再内存计数。"""
        now = get_now()
        today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
        week_start = today_start - timedelta(days=today_start.weekday())
        month_start = today_start.replace(day=1)

        base_filters = self._build_record_filters(email, code, team_id, start_date, end_date)

        async def count_with_boundary(boundary: Optional[datetime] = None) -> int:
            filters = list(base_filters)
            if boundary:
                filters.append(RedemptionRecord.redeemed_at >= boundary)

            stmt = select(func.count()).select_from(RedemptionRecord)
            if filters:
                stmt = stmt.where(and_(*filters))
            result = await db_session.execute(stmt)
            return result.scalar() or 0

        total = await count_with_boundary()
        today = await count_with_boundary(today_start)
        this_week = await count_with_boundary(week_start)
        this_month = await count_with_boundary(month_start)
