...
DEFAULT_TEAM_BAN_POLL_INTERVAL_MINUTES = 30
MIN_TEAM_BAN_POLL_INTERVAL_MINUTES = 1
MAX_TEAM_BAN_POLL_INTERVAL_MINUTES = 1440
TEAM_BAN_POLL_INTERVAL_SETTING_KEY = "team_ban_poll_interval_minutes"
...
# 创建全局实例
    async def get_team_ban_poll_interval_minutes(self, session: AsyncSession) -> int:
        """获取 Team 封禁巡检轮询间隔(分钟)。"""
        raw_value = await self.get_setting(
            session,
            TEAM_BAN_POLL_INTERVAL_SETTING_KEY,
            str(DEFAULT_TEAM_BAN_POLL_INTERVAL_MINUTES)
        )
        ...
        try:
            ...
            logger.warning(
                "封禁巡检轮询间隔配置无效: %s, 使用默认值 %s 分钟",
                raw_value,
                DEFAULT_TEAM_BAN_POLL_INTERVAL_MINUTES,
            )
            return DEFAULT_TEAM_BAN_POLL_INTERVAL_MINUTES

        if interval < MIN_TEAM_BAN_POLL_INTERVAL_MINUTES or interval > MAX_TEAM_BAN_POLL_INTERVAL_MINUTES:
            logger.warning(
                "封禁巡检轮询间隔超出范围: %s, 使用默认值 %s 分钟",
                interval,
                DEFAULT_TEAM_BAN_POLL_INTERVAL_MINUTES,
            )
            return DEFAULT_TEAM_BAN_POLL_INTERVAL_MINUTES
    ...
    async def update_team_ban_poll_interval_minutes(self, session: AsyncSession, interval_minutes: int) -> bool:
        """更新 Team 封禁巡检轮询间隔(分钟)。"""
        if interval_minutes < MIN_TEAM_BAN_POLL_INTERVAL_MINUTES or interval_minutes > MAX_TEAM_BAN_POLL_INTERVAL_MINUTES:
            logger.error(
                "封禁巡检轮询间隔无效: %s, 允许范围为 %s-%s 分钟",
                interval_minutes,
