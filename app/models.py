...
class LinuxDoCreditOrder(Base):
    """Linux.do 积分订单表"""
    __tablename__ = "linuxdo_credit_orders"
    ...
    money = Column(String(32), nullable=False, comment="积分数量")
    redeem_code = Column(String(32), comment="用于复用兑换流程的伪兑换码")
    pay_url = Column(Text, comment="支付链接")
    trade_no = Column(String(64), comment="平台订单号")
    status = Column(String(20), default="pending", comment="订单状态 pending/paid/fulfilled/failed")
    created_at = Column(DateTime, default=get_now, comment="创建时间")
    updated_at = Column(DateTime, default=get_now, onupdate=get_now, comment="更新时间")

    __table_args__ = (
        Index("idx_linuxdo_order_no", "out_trade_no"),
        Index("idx_linuxdo_order_email", "email"),
    )
